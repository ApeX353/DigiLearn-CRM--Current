...
import { BackfillDisqualifyReasonDialog } from "~/components/leads/lead-actions/backfill-disqualify-reason-dialog";
...
export default function LeadsManagementPage() {
  ...
  // Backfill target: the Disqualified row whose "Reason needed" chip was
  // clicked. Null closes the dialog.
  const [backfillLead, setBackfillLead] = useState<Lead | null>(null);
  ...
  // Only fetched while the Disqualified tab is open — the strip that
  // answers "why were all these leads disqualified" lives there.
  const { data: dqSummary } = useDisqualificationSummary(
  ...
  const leadColumns = useMemo<ColumnDef<Lead>[]>(
    () => [
      ...
      ...baseLeadColumns,
      // The Disqualified tab exists to answer WHY, so the reason rides
      // on the row. A row with none gets the chip that opens the
      // backfill dialog — that queue is how the gap gets worked off.
      ...(leadFilter === "Disqualified"
        ? ([
            {
              id: "reason",
              header: "Why disqualified",
              cell: ({ row }: { row: { original: Lead } }) => {
                ...
                return reason ? (
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-6 border-amber-400 px-2 text-[11px] text-amber-700 hover:text-amber-800"
                    onClick={(e) => {
                      e.stopPropagation();
                      setBackfillLead(row.original);
                    }}
                  >
                    Reason needed
                  </Button>
  ...
  return (
    <div>
      ...
      <section className="space-y-2 px-3 md:px-4 pb-4 pt-1">
        ...
        {leadFilter === "Disqualified" && (
          <p className="rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">
              Archived — disqualified leads.
            </span>{" "}
            A disqualified lead is no longer a potential deal, so it leaves
            All Leads, the totals, the SLA alerts and the lead pickers. It is
            kept here — not deleted — so managers can review why it was
            closed (cut too early? should have been nurtured? reassigned?)
            and reverse it through the approval flow if it was.
          </p>
        )}
        {leadFilter === "Disqualified" && dqSummary && (
          <div className="flex flex-wrap items-center gap-1.5 rounded-md border bg-muted/30 px-3 py-2 text-xs">
            <span className="font-medium text-muted-foreground">
              Why {dqSummary.total} leads were disqualified:
            </span>
            {dqSummary.reasons.map((r) => (
              <span
                key={r.reason}
            ...
            {dqSummary.missingReason > 0 && (
              <span className="rounded-full border border-amber-400 bg-amber-50 px-2 py-0.5 text-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
                no reason recorded ×{dqSummary.missingReason}
              </span>
            )}
          </div>
        )}
      ...
      <BackfillDisqualifyReasonDialog
        lead={backfillLead}
        open={!!backfillLead}
        onOpenChange={(open) => {
          if (!open) setBackfillLead(null);
        }}
      />
      <AssignLeadsDialog
