...
const EVENT_TYPE_GROUPS = [
  ...
  // FU6: backed by FollowupReminderService (server cron) — the toggles now
  // gate real lead/deal follow-up due & missed notifications.
  {
    label: "Follow-ups",
...
function SettingsPageContent() {
  ...
  useEffect(() => {
    if (generalSettings) {
      ...
      setFiscalYear(String(generalSettings.default_fiscal_year || ""));
    }
  }, [generalSettings]);
  ...
  const handleSaveCurrencyLocale = async () => {
    try {
      await saveSettings({
        settings: [
          // 6.4: currency MUST be public — the whole app (useCurrency) and PDFs
          // read it; without is_public it is filtered out of /settings/public
          // and the UI silently falls back to USD while documents use the real
          // value. Timezone is public too so a future locale feature can read it.
          { key: "currency", value: currency, data_type: "string", is_public: true },
          { key: "timezone", value: timezone, data_type: "string", is_public: true },
          // Fiscal-year start — consumed server-side by the payments fiscal-year
          // logic; this is the editor that was missing so it could never be set.
          {
        ...
        <TabsContent value="general" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Name, email, phone and website appear on generated invoices and
                quotations.
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="e.g. 2026 or 2026-04-01"
                      />
                      <p className="text-xs text-muted-foreground">
                        Year or start date. Used by fiscal-year reporting.
                      </p>
                    </div>
                  </div>
                  <Button
