...
export default function ActivitiesPage() {
  ...
  // Opens on "Due today" (25 Aug, on request). This REVERSES an earlier
  // decision to default to the full open working list, whose reasoning was
  // that a task due tomorrow is filtered out of the landing view — that
  // trade-off is real and still applies: work that is not due today is one
  // click away on the To-do tab rather than in front of the rep on arrival.
  // The ask is that the page answers "what do I owe today?" first.
  //
  // FU2: the period still hydrates from the URL, so the dashboard "Nurture
  // Follow-ups → View all" link can land on /activities?period=overdue with
  // the overdue tab already active (mirrors the ?type= hydration below).
  const urlPeriod = searchParams.get("period");
  const initialPeriod: ActivitiesPeriod =
    urlPeriod && ACTIVITIES_PERIODS.some((p) => p.value === urlPeriod)
      ? (urlPeriod as ActivitiesPeriod)
      : "today";
  const [period, setPeriod] = useState<ActivitiesPeriod>(initialPeriod);
  /**
            ...
            <ul className="divide-y rounded-lg border bg-background">
              {visibleActivities.map((activity) => (
                <CompletedActivityFeedItem
                  ...
                  // This list is a work queue, not a record's own log:
                  // the point of a row is the school/lead/deal behind it,
                  // so opening one takes you there. Reading a long note
                  // still expands in place — that click is on the note.
                  onOpenRelated={(a: Activity) => {
                    const target = activityRelatedTarget(a);
