...
export default function CampaignDetailPage() {
  ...
  const {
    ...
    isError: roiError,
    refetch: refetchRoi,
  } = useCampaignRoi(canSeeRoi ? campaignId : "");
  ...
  // CAMP-UI-ERR: previously `!campaign` was folded into the loading branch, so
  // a failed load (404/500/permission) left the page spinning forever with no
  // explanation. Show an actual error state with a way out.
  if (campaignError || !campaign) {
    return (
      <Container>
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          <p className="text-sm text-muted-foreground">
            This campaign could not be loaded. It may have been removed, or you
            may not have access.
          </p>
  ...
  return (
    <Container>
      ...
      <div className="space-y-4">
        {/* CAMP-UI-ERR: a failed ROI request used to make the whole card
            disappear silently. Show an error card with a retry instead. */}
        {canSeeRoi && roiError && (
          <Card data-testid="campaign-roi-error">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <TrendingUp className="h-4 w-4" /> Campaign ROI
              </CardTitle>
              <CardDescription>
                ROI could not be loaded.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button variant="outline" size="sm" onClick={() => refetchRoi()}>
                Retry
              </Button>
            </CardContent>
          </Card>
                        ...
                        {roi.won_deal_costs.map((d) => (
                          <tr key={`${d.deal_id}-${d.currency}`}>
                            ...
                            <td className="px-3 py-2 text-right whitespace-nowrap">
                              {money(d.deal_value, d.deal_currency)}
                            </td>
                            <td className="px-3 py-2 text-right whitespace-nowrap">
