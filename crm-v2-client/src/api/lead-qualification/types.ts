...
export const leadQualificationSchema = z
  .object({
    ...
    //
    // Optional at the schema level and enforced in the form instead,
    // because the requirement is only satisfiable when an admin has
    // configured payment plans. With none set up the picker has nothing
    // to offer, and a hard requirement here made qualification — and so
    // conversion — impossible for every lead in the system rather than
    // telling anyone what was missing.
