...
const settingsApi = {
  ...
  // C6: the server wraps the map in a { success, data } envelope. Unwrap here
  // so consumers can read `settings[key]` directly — otherwise every lookup is
  // undefined, the admin tabs seed from code defaults, and saving the
  // Compliance tab silently resets the 20 keys the user didn't touch.
  getSettings: (keys: string[]): Promise<SettingsResponse> =>
    apiClientAuth
      .get("/settings", { params: { keys: keys.join(",") } })
      .then((res) => (res.data?.data ?? res.data) as SettingsResponse),
