...
// DEAL-COMP-400: the deal form collects competitors in a UI-local shape
// ({ competitorName, competitorStrength, status, ourAdvantage, theirAdvantage })
// but the server's CreateDealCompetitorDto expects { name, strengths,
// weaknesses, threat_level, notes, price }. The global ValidationPipe runs
// with forbidNonWhitelisted, so posting the UI shape 400s the ENTIRE create
// ("property competitorName should not exist" + "name should not be empty").
// Any deal saved with a competitor row failed. Map to the server shape here so
// every caller (create + update) is covered; rows already in server shape pass
// through untouched.
const toServerCompetitors = (competitors: any): any => {
  if (!Array.isArray(competitors)) return competitors;
  return competitors.map((c) => {
    if (c && "competitorName" in c) {
      return {
        name: c.competitorName,
        ...(c.theirAdvantage ? { strengths: c.theirAdvantage } : {}),
        ...(c.ourAdvantage ? { weaknesses: c.ourAdvantage } : {}),
        ...(c.status ? { threat_level: String(c.status) } : {}),
        ...(typeof c.competitorStrength === "number"
          ? { notes: `Strength: ${c.competitorStrength}%` }
          : {}),
      };
    }
    return c;
...
const withServerCompetitors = <T extends { competitors?: any }>(data: T): T =>
  data && "competitors" in data
    ? { ...data, competitors: toServerCompetitors(data.competitors) }
    : data;
...
const api = {
  ...
  create: (data: CreateDealDto): Promise<Deal> =>
    apiClientAuth
      .post("/deals", withServerCompetitors(data))
      .then((r) => r.data),

  update: (id: string, data: UpdateDealDto): Promise<Deal> =>
    apiClientAuth
      .put(`/deals/${id}`, withServerCompetitors(data))
      .then((r) => r.data),
