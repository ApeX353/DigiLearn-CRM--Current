...
type LeadsListParams = PaginationParams & {
  ...
  school_id?: string;
  /**
   * Archive rule: an unfiltered list is ACTIVE potentials only — the
   * server leaves Disqualified leads out unless asked. Pass true where
   * history matters (a school's full lead history, exports); the Archive
   * tab asks for status=Disqualified explicitly.
   */
...
export const leadsKeys = {
  ...
  disqualificationSummary: () =>
    [...leadsKeys.all, "disqualification-summary"] as const,
};
...
const leadsApi = {
  ...
  // Edit an existing lead stakeholder. PATCH, so it carries only what changed.
  updateStakeholder: (
    leadId: string,
    stakeholderId: string,
    data: Partial<AddLeadStakeholderPayload>,
  ): Promise<ApiResponse<LeadStakeholder>> =>
    apiClientAuth
      .patch(`/leads/${leadId}/stakeholders/${stakeholderId}`, data)
      .then((res) => res.data),
};
 ...
 */
/**
 * Why leads were disqualified — per-reason counts plus how many rows
 * carry no reason (the backfill queue). Powers the breakdown strip on
...
export function useDisqualificationSummary(enabled: boolean) {
  return useQuery({
    queryKey: leadsKeys.disqualificationSummary(),
    queryFn: () =>
      apiClientAuth
        .get("/leads/disqualification-summary")
        .then((res) => res.data.data as DisqualificationSummary),
    staleTime: 60 * 1000,
    enabled,
  });
}
 ...
 */
export function useUpdateLeadStakeholder(leadId: string) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      stakeholderId: string;
      data: Partial<AddLeadStakeholderPayload>;
    }) => leadsApi.updateStakeholder(leadId, stakeholderId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: leadsKeys.all });
