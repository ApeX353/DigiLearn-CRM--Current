...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        // CRM density pass — h-9 → h-8 to match the new Button scale
        // and text-base → text-sm so form fields read at the same weight
        // as surrounding labels/values instead of bulging 1 size larger.
