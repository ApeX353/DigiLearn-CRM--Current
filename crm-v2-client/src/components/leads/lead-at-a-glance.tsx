...
// pickPivotalActivities is defined locally below in his version — it adds the
// rule that a note is not a "touch". The shared activity-kit helper is still
// used by the other surfaces that import it.
import {
  ActivitySignalLine,
  ActivityTypeIcon,
  activityTouchDate,
      ...
      >
        ...
        >
          <SectionRow icon={Target} label="Status">
            ...
            {(lead.status === "Disqualified" || lead.status === "Nurture") &&
              (lead.reason ?? "").trim() !== "" && (
...
function pickPivotalActivities(activities: Activity[]) {
  ...
  // Notes are born completed now, but writing a note is not touching the
  // school — "Last touch" stays a real interaction.
  const completed = activities
    .filter((a) => a.status === "completed" && a.type !== "note")
