...
export default function PaymentTermsCalculatorModal({
  ...
  return (
    ...
    >
      <Form {...form}>
        {/* Nothing to calculate with until an admin configures the
            firm's plans. Without this the term picker just opened onto
            an empty list and the rep was left clicking a control that
            appeared broken. */}
        {paymentTerms.length === 0 && (
          <p className="mb-4 rounded-md border border-dashed p-3 text-sm text-muted-foreground">
            No payment plans are configured yet, so there is nothing to
            apply. An admin can add your instalment terms under{" "}
