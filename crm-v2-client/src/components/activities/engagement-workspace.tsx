...
interface EngagementWorkspaceProps {
  ...
  isReadonly?: boolean;
  /**
   * Who a composed activity should be assigned to by default. The lead and
   * deal activity tabs pass the record owner so work logged on someone's
   * record lands on them, not on whoever typed it.
   */
...
export function EngagementWorkspace({
  ...
  const {
    ...
    ...queryParent,
    open_only: true,
    // 50, not 25: records polluted by legacy undated "open" rows were
    // silently truncating genuine planned work out of the Next-step slot.
    limit: 50,
    page: 1,
  ...
  //
  // DATED items only. The previous fallback — "no dated item? show the
  // most recently created open one" — put whatever was logged last into
  // the Next-step slot: on records polluted by legacy undated rows that
  // was a call that already happened, presented as the planned future,
  // while the sidebar's "Next activity" (which only reads dated items)
  // named something else. A plan without a date is not a next step; it
  // stays in the log flagged "Needs a date" until someone dates it.
  ...
  // Open actionable items beyond the one occupying the Planned slot.
  const otherOpenCount = useMemo(() => {
    const open = (openData?.data ?? []).filter((a) => a.type !== "note");
    return plannedActivity ? Math.max(0, open.length - 1) : open.length;
  }, [openData, plannedActivity]);
  ...
  const feedActivities = useMemo<Activity[]>(() => {
    ...
    // Full timeline: every activity on this record EXCEPT the single item
    // highlighted in the Planned "Next step" slot. Previously this showed only
    // completed items + open notes, so an open task/call that wasn't the
    // soonest next-step was invisible — it was neither the planned card nor
    // here. Now nothing a rep logs or schedules can disappear.
    //
    ...
    const combined = [...byId.values()].filter(
      (a) => a.id !== plannedActivity?.id,
    );
  ...
  }, [doneData, openData, cancelledData, plannedActivity, feedFilter]);
        ...
        {loadingOpen ? (
          ...
          <>
            <PlannedActivityCard
              ...
              expanded={expandedId === plannedActivity.id}
              onComplete={toggleComplete}
              onOpen={openInspector}
              disabled={isReadonly}
            />
            {/* The slot shows ONE next step by design, but reps completing
                it were surprised by "new" planned items appearing — they
                were just next in an invisible queue. Name the queue. */}
            {otherOpenCount > 0 && (
              <p className="mt-1.5 text-center text-xs text-muted-foreground">
                +{otherOpenCount} more open{" "}
                {otherOpenCount === 1 ? "activity" : "activities"} on this
                record — they're in the log below and surface here one at a
                time.
              </p>
            )}
          </>
            ...
            {timelineItems.map((item) =>
              item.kind === "activity" ? (
                <CompletedActivityFeedItem
                  ...
                  onOpen={openInspector}
                  onReopen={
                    // A note is a record — "reopening" one would turn it
                    // into phantom open work.
                    item.activity.status === "completed" &&
                    item.activity.type !== "note"
