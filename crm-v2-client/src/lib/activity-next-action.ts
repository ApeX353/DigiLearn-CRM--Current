...
/**
 * Exempt by product rule:
 *  - notes — record-keeping only;
 *  - anything completed or cancelled — a finished interaction is HISTORY.
 *    The next step is owed by the record (lead/deal), and lives in its own
 *    open dated activity; stamping "No follow-up" on every past call made
 *    a rep's actual work read as a string of failures.
 */
export function isNextActionExempt(activity?: Activity | null): boolean {
  if (!activity) return true;
  if (activity.type === "note") return true;
  return (
