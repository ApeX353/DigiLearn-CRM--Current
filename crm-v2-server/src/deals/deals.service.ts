...
export class DealsService {
  ...
  private async assertCommercialIntentGate(
    ...
    // ---- commercial intent: three ways to satisfy it, not one ----------
    //
    // The switch was unsatisfiable in practice. commercial_intent is only
    // ever set by completing a DEMO activity carrying a recorded signal,
    // and on the 20 Aug production copy there are ZERO demo activities of
    // any kind: 1,181 open rep-owned leads all have commercial_intent
    // false and not one has it true. With the switch on, no sales rep
    // could convert any lead at all — the Ruvheneko ticket was a symptom
    // of that, not a data quirk.
    //
    // Widening the automatic rule does not rescue it either: on the same
    // data, accepting any recorded commercial outcome unblocks 3 leads and
    // accepting BANT-qualified-with-a-decision-maker unblocks 16. The team
    // simply does not record the evidence this gate was written to look
    // for.
    //
    // So the control keeps its teeth but becomes satisfiable: the rep may
    // DECLARE the intent and say why. The declaration is stored on the
    // lead and written to the audit log, so "who said this was live, and
    // on what basis" is answerable — which is the thing the gate actually
    // exists to guarantee. Silence still fails.
    const declaredReason = dto.commercial_intent_reason?.trim();

    if (!lead.commercial_intent && !declaredReason) {
      const evidence = await this.findCommercialEvidence(lead.id);
      if (!evidence) {
        missing.push(
          'commercial intent — either record the signal (an activity closed as Quote Requested, Strong Interest, Proposal Sent, SDC Meeting or Payment Plan Requested, Interested, or Waiting for Approval; or complete qualification with a decision-maker), or state why this lead is commercially live in the reason box',
        );
      }
    }
  ...
  /**
   * Evidence that a lead is commercially live, drawn from what reps actually
   * record rather than from the demo workflow alone.
   *
   * Either a completed activity closed with a commercial outcome, or a
   * finished qualification naming a decision-maker. Returns a short label
   * for the audit trail, or null when there is nothing.
   */
  private async findCommercialEvidence(leadId: string): Promise<string | null> {
    ...
    const activity = await this.dataSource
      .getRepository('activities' as any)
      .createQueryBuilder('a')
      .where('a.lead_id = :leadId', { leadId })
      .andWhere("a.status = 'completed'")
      .andWhere('a.completion_outcome IN (:...outcomes)', {
        outcomes: COMMERCIAL_OUTCOMES,
      })
      .getRawOne()
      .catch(() => null);
    if (activity) return `recorded outcome ${activity.a_completion_outcome}`;

    const qual = await this.dataSource
      .getRepository('lead_qualification_criteria' as any)
      .createQueryBuilder('q')
      .where('q.lead_id = :leadId', { leadId })
      .andWhere('q.is_qualified = true')
      .getRawOne()
      .catch(() => null);
    if (
      qual &&
      ...
      return 'qualification complete with a decision-maker';
    }
  ...
  async createDeal(
    ...
    // A declared intent has to leave a trace, or the control is theatre.
    // Record it on the lead and in the audit log so "who said this lead was
    // commercially live, when, and on what basis" is answerable later.
    // Only on the way in, and only when the lead had not already earned it.
    const declaredIntent = dto.commercial_intent_reason?.trim();
    if (declaredIntent && !lead.commercial_intent) {
      await this.leadRepository.update(lead.id, {
        commercial_intent: true,
        commercial_intent_at: new Date(),
        commercial_intent_reason: `declared_by_rep: ${declaredIntent}`,
      } as any);
      await this.activityLogsService.logUpdate(
        'Lead',
        lead.id,
        { commercial_intent: false },
        { commercial_intent: true, reason: declaredIntent },
        userId,
        `Commercial intent declared when creating a deal: ${declaredIntent}`,
      );
    }
    ...
    // One order, one deal (recording rule 6). A school with an open deal
    // almost always means the rep is about to duplicate it — Errymaple
    // ended up on the bank order book twice this way. A genuine second
    // order passes duplicate_override=true, which the client sets only
    // after showing the rep the existing deal and getting an explicit
    // "create another".
    if (!dto.duplicate_override) {
      const openDeal = await this.dealRepository.findOne({
        where: {
      ...
      if (openDeal) {
        throw new ConflictException({
          message:
            `${school.name} already has an open deal: "${openDeal.title}" ` +
            `(${openDeal.currentStatus}, ${Number(openDeal.value).toLocaleString('en-US')}). ` +
            'If this is genuinely a second order, confirm to create another deal.',
          code: 'DUPLICATE_OPEN_DEAL',
          existing_deal: {
            id: openDeal.id,
            title: openDeal.title,
            stage: openDeal.currentStatus,
            value: Number(openDeal.value),
          },
        });
      }
    }
  ...
  async closeDeal(
    ...
    // Closing moves the CARD, not just the status. "Mark Won" used to set
    // close_status and leave the deal standing mid-pipeline, so the board
    // counted closed revenue as live work (four production deals sat in
    // Delivery with close_status='won'). Landing on a terminal stage
    // already flips close_status (see buildStageTransitionUpdatePayload);
    // this is the reverse direction, using the same transition machinery
    // so stage history, positions and "Advance …" task auto-completion
    // all behave exactly as a drag would.
    let closeMessage = `Closed deal "${deal.title}" as ${dto.close_status}`;

    await this.dataSource.transaction(async (manager) => {
      // Only active stages — a retired stage must not become a dumping
      // ground for closed deals.
      const stages = await manager.find(Stage, {
        where: { pipeline_id: deal.pipeline_id, is_active: true },
        order: { order: 'DESC' },
      });
      const targetStage = stages.find(
        (s) => this.getTerminalStageKind(s.name) === dto.close_status,
      );

      const closeFields: Partial<Deal> = {
        closeStatus: dto.close_status,
        actualCloseDate: new Date(),
        lostReason:
          dto.close_status === DealCloseStatus.LOST
            ? dto.lost_reason
            : undefined,
      };
      ...
      if (targetStage && targetStage.id !== deal.current_stage_id) {
        const transitionUpdate = await this.buildStageTransitionUpdatePayload(
          manager,
          {
            ...
            targetStage,
            actorId: userId,
            notes: `Closed as ${dto.close_status}`,
          },
        );
        await manager.update(
          Deal,
          { id },
          { ...transitionUpdate, ...closeFields },
        );
        closeMessage = `Closed deal "${deal.title}" as ${dto.close_status} and moved it to "${targetStage.name}"`;
      } else {
        // The seeded production pipeline has no Lost stage. Refusing to
        // close would be worse than leaving the card where it stands.
        await manager.update(Deal, { id }, closeFields);
        if (!targetStage) {
          closeMessage = `Closed deal "${deal.title}" as ${dto.close_status} — no ${dto.close_status} stage in this pipeline, the card stays on "${deal.currentStatus}"`;
        }
      }

      // A LOST deal is a dead end — cancel its open activities so the loss
      // doesn't keep feeding reps' To-do queues. WON deals keep their queue:
      // delivery, installation and training work legitimately continues.
      if (dto.close_status === DealCloseStatus.LOST) {
        const cancelled = await manager
          .createQueryBuilder()
          .update(Activity)
          .set({ status: ActivityStatus.CANCELLED })
          .where('deal_id = :id', { id })
          .andWhere('status NOT IN (:...closed)', {
            closed: [ActivityStatus.COMPLETED, ActivityStatus.CANCELLED],
          })
          // Notes are records of what was observed, not scheduled work —
          // cancelling one hides it from the record's history feed.
          .andWhere('type != :note', { note: ActivityType.NOTE })
          .execute();
        if (cancelled.affected) {
          await this.activityLogsService.logUpdate(
            ...
            id,
            {},
            { cancelled_open_activities: cancelled.affected },
            userId,
            `Cancelled ${cancelled.affected} open activit${
              cancelled.affected === 1 ? 'y' : 'ies'
            } on lost deal`,
          );
        }
      }
  ...
  async autoCreateDealForQuotedLead(
    ...
    // Resolve the lead from the SCHOOL, and judge ambiguity there.
    //
    // This used to narrow by the quote's contact first and then check that
    // exactly one candidate came back — which manufactured false certainty.
    // A school with several open leads is ambiguous whether or not they
    // share a contact, and schools routinely have one contact across every
    // lead. Ingwalo Secondary had two open leads sharing one contact; a
    // quote raised for "Supply Of Interactive Boards" resolved to
    // "Intelligence system: LMS, SMS, ERP" and silently converted it,
    // twice (17 Aug, QUO-2026-0080). The rep could then log no activity
    // against a lead the system had closed underneath them.
    //
    // Terminal leads are excluded from the count: a Disqualified lead is
    // not a candidate for conversion, and letting one sit in the set both
    // blocked good resolutions and risked reviving a dead lead.
    const openLeads = await manager.find(Lead, {
      where: {
        school_id: schoolId,
        status: Not(In(['Converted', 'Disqualified'])) as any,
      },
    });

    // Exactly one open lead makes the resolution reliable; zero or an
    // ambiguous match means we do nothing rather than guess. The quote is
    // simply left standalone and the rep converts the lead they meant.
    if (openLeads.length !== 1) return null;
    const lead = openLeads[0];

    // When the quote names a contact, it has to agree with the lead we
    // resolved. Disagreement means the quote is about someone else at the
    // school, which is exactly the case we must not guess at.
    if (
      personId &&
      lead.primary_contact_id &&
      lead.primary_contact_id !== personId
    ) {
      return null;
    }
