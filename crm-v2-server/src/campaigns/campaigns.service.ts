...
export class CampaignsService {
  ...
  async update(id: string, dto: UpdateCampaignDto): Promise<Campaign> {
    ...
    // CAMP-ENTRY-EDIT: entry_date is editable (a campaign logged late carries
    // its true intake date), but update() never applied it — the save
    // succeeded while the date silently stayed put. Persist it like the rest.
    if (dto.entry_date !== undefined) campaign.entry_date = dto.entry_date ?? null;
    return this.campaignRepo.save(campaign);
  }
