...
export class ActivityDisciplineService {
  ...
  private async computeProspecting(
    ...
    // --- KPI 1: distinct leads contacted TODAY ----------------
    // "40 a day" counts one per LEAD per day: logging a call, WhatsApp,
    // meeting etc. on a lead counts once, and a second activity on the SAME
    // lead the same day does NOT count again (COUNT(DISTINCT lead_id), not
    // COUNT(id)). New and existing leads both count.
    //
  ...
  private async computeAtRisk(uid?: string): Promise<AtRiskItem[]> {
    ...
    const slaBreachedQ = this.leadRepo
      ...
      // converted leads owe no response.
      .andWhere("l.status NOT IN ('Disqualified','Converted')");
      ...
      .andWhere(
        `NOT EXISTS (
          ...
          WHERE a.lead_id = l.id
            ...
            AND a.type IN ('task','call','email','meeting','whatsapp')
            AND (a.due_at IS NOT NULL OR a.scheduled_at IS NOT NULL)
        )`,
      );
