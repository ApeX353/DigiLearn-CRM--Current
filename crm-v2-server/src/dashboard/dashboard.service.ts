...
export class DashboardService {
  ...
  async getExecutiveKPIs(
    ...
    const qualQb = this.qualificationRepo
      ...
      // Archive rule: a disqualified lead's qualification row is not a
      // potential — keep it out of totals and averages.
      .andWhere("lead.status <> 'Disqualified'");
  ...
  async getLeadsByStage(
    ...
    const qb = this.leadRepo
      ...
      // Archive rule: the stage wheel is a picture of live potentials; the
      // widget sums every slice into "total leads", so a Disqualified
      // slice would count dead records as potential. Archived leads are
      // reviewed on the Leads page Archive tab instead.
      .andWhere("l.status <> 'Disqualified'");
  ...
  async getNurtureFollowUps(
    ...
    // FU1: this card is "Nurture Follow-ups", so the truthful population is
    // open actionable follow-ups that belong to a lead currently in Nurture.
    // The previous query joined the lead loosely and never filtered on it, so
    // it also counted deal-only work and work on Qualified/Converted/
    // Disqualified leads. Requiring the lead (inner join) + status = 'Nurture'
    // drops both. We also key off the EFFECTIVE date COALESCE(due_at,
    // scheduled_at) so legacy scheduled_at-only rows are not silently omitted.
    const EFFECTIVE_DATE = 'COALESCE(a.due_at, a.scheduled_at)';
    const applyNurturePopulation = (
      qb: import('typeorm').SelectQueryBuilder<Activity>,
    ) => {
      qb.andWhere('a.type IN (:...actionableTypes)', {
        ...
        .andWhere('lead.status = :nurture', { nurture: 'Nurture' });
      if (userRole === 'sales_rep')
        qb.andWhere('a.assigned_to_id = :uid', { uid: userId });
      if (filters.salesRepId)
        qb.andWhere('a.assigned_to_id = :rep', { rep: filters.salesRepId });
      if (filters.province)
        qb.andWhere('school.province = :prov', { prov: filters.province });
      return qb;
    };

    // Overdue (effective date already passed)
    const overdueQb = applyNurturePopulation(
      this.activityRepo
        .createQueryBuilder('a')
        .innerJoinAndSelect('a.lead', 'lead')
        .leftJoin('lead.school', 'school')
        .leftJoinAndSelect('a.assigned_to', 'assignee')
        .where(`${EFFECTIVE_DATE} < :now`, { now }),
    );

    // Uncapped total, computed separately from the capped preview list (FU1:
    // the old code returned overdue.length after .take(20), pinning the count
    // at 20).
    const overdueCount = await overdueQb.getCount();
    // Use .limit() not .take(): .take() with joinAndSelect builds a
    // "SELECT DISTINCT id … ORDER BY <expr>" pagination sub-query, and
    // Postgres rejects ordering by a COALESCE() expression that is not in the
    // DISTINCT select list. Both joins here are to-one, so a raw LIMIT returns
    // the correct number of activities and allows the ORDER BY expression.
    ...
    // Due today
    const dueTodayQb = applyNurturePopulation(
      this.activityRepo
        .createQueryBuilder('a')
        .innerJoinAndSelect('a.lead', 'lead')
        .leftJoin('lead.school', 'school')
        .leftJoinAndSelect('a.assigned_to', 'assignee')
        .where(`${EFFECTIVE_DATE} >= :start`, { start: this.startOfDay(now) })
        .andWhere(`${EFFECTIVE_DATE} <= :end`, { end: todayEnd }),
    );

    const dueToday = await dueTodayQb.getCount();

    // Due in next 7 days
    const dueWeekQb = applyNurturePopulation(
      this.activityRepo
        .createQueryBuilder('a')
        .innerJoin('a.lead', 'lead')
        .leftJoin('lead.school', 'school')
        .where(`${EFFECTIVE_DATE} > :today`, { today: todayEnd })
        .andWhere(`${EFFECTIVE_DATE} <= :week`, { week: in7 }),
    );

    const dueIn7Days = await dueWeekQb.getCount();

    const formatActivity = (a: Activity) => {
      const effective = a.due_at ?? a.scheduled_at ?? null;
      return {
        id: a.id,
        type: a.type,
        subject: a.subject,
        ...
        assigneeName: a.assigned_to
          ? `${a.assigned_to.first_name} ${a.assigned_to.last_name}`
          : 'Unassigned',
        dueAt: effective?.toISOString() || null,
        daysOverdue: effective ? this.diffDays(now, effective) : 0,
      };
    };
