    ...
    try {
      ...
      for (const slaConfig of activeSlaConfigs) {
        ...
        for (const lead of leads) {
          ...
          //
          // The row is matched on the lead AND the status, which is right
          // when the history is tidy. It often is not: 166 of 719 open
          // history rows on production carry a status the lead has since
          // left, because only transitionStatus closes the old row and
          // imports, direct writes and the reversal-approval path do not.
          // A lead that breached under New and then moved to Contacted has
          // an open breached row saying New and none saying Contacted, so
          // this lookup found nothing and the loop skipped it — silently,
          // forever. Sixteen production leads sat breached and never once
          // escalated, some since March.
          //
          // So: prefer the row for the current status, and fall back to the
          // most recent open breached row for the lead whatever status it
          // names. The breach is real either way — the lead carries
          // sla_breached itself, which is what put it in this list.
          const history =
            (await this.leadSlaHistoryRepository.findOne({
              where: {
                lead_id: lead.id,
    ...
    try {
      ...
      for (const slaConfig of activeSlaConfigs) {
        ...
        for (const lead of leads) {
          ...
          // Find the open history row for this lead+status. This must NOT
          // filter on idle_alert_sent: that flag is what we are here to set,
          // so requiring it made the lookup a bootstrap deadlock — the row
          // could only be found once it already carried the flag, nothing
          // ever set it, and the guard below therefore never engaged. 0 of
          // 1,225 production history rows had it set; the alert fired
          // ~11,000 times a day, 294,799 rows in 30 days.
          ...
          if (
            existingHistory &&
            existingHistory.idle_alert_sent &&
          ...
          // Day bucket, not hour. The scheduler runs every 2 hours, so an
          // hour-sliced key never collided and dedupe suppressed nothing.
          // Belt and braces alongside the history guard above.
          const dedupeKey = `lead-idle-alert-${lead.id}-${now.toISOString().slice(0, 10)}`;
          try {
            await this.userNotificationsService.sendToUsers({
          ...
          // Record that the alert went out. Creating the row when none
          // exists is the half that was missing: without it the guard above
          // had nothing to find on the next run and re-alerted forever.
          if (existingHistory) {
            ...
            await this.leadSlaHistoryRepository.save(
              this.leadSlaHistoryRepository.create({
                lead_id: lead.id,
                status: slaConfig.status,
                entered_status_at: lead.last_action_at ?? now,
                exited_status_at: null,
                sla_due_date: idleThreshold,
                sla_hours: slaConfig.idle_alert_hours,
                sla_met: false,
                sla_breached: false,
                escalated: false,
                idle_alert_sent: true,
