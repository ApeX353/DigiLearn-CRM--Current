...
export class QueryLeadDto {
  ...
  @ApiPropertyOptional({
    description:
      'Include ARCHIVED leads (status = Disqualified) in an unfiltered list. A disqualified lead is not a potential deal, so by default it leaves the active list, the Total count and SLA alerts; it stays reachable by id, and by asking for status=Disqualified (the Archive tab) or include_archived=true.',
  })
  @IsOptional()
  @Type(() => String)
  @Transform(({ value }) => toBool(value))
