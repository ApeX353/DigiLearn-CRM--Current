...
export class LeadsService {
  ...
  async findAll(
    ...
    if (status) {
      ...
      // ARCHIVE RULE. "A disqualified lead isn't a lead. Lead means a
      // potential deal; once disqualified that potential is lost." So the
      // unfiltered list — All Leads, the Total count, the lead pickers on
      // activities and deals — shows active potentials only. Disqualified
      // leads are archived, not deleted: they stay reachable by id, on the
      // Archive tab (status=Disqualified), and in the managers' reason
      // review. Nothing is soft-deleted, so the approval queue, the
      // disqualification summary and the detail page keep working.
      queryBuilder.andWhere("lead.status <> 'Disqualified'");
    }
    ...
    if (sla_breached !== undefined) {
      ...
      // "Show Breached Leads" must agree with the banner it sits under:
      // a closed record cannot be late. Only when no explicit status was
      // asked for — status=Converted&sla_breached=true stays honest.
      if (sla_breached && !status) {
        queryBuilder.andWhere(
          "lead.status NOT IN ('Disqualified','Converted')",
        );
      }
    }
  ...
  async getStatusCounts(
    ...
    // Archive rule: "All" is the active list — disqualified leads are
    // archived potentials, counted under their own bucket (and echoed as
    // Archived) but not as part of All.
    counts.Archived = counts.Disqualified ?? 0;
    counts.All = total - counts.Archived;
    return counts;
  }
  ...
  async findOne(id: string, ability?: AppAbility): Promise<Lead> {
    ...
    // Carry the open approval request on the lead itself.
    //
    // The request row existed but nothing on the lead said so, which meant a
    // manager had to go to the queue to discover that this lead was waiting
    // on them — the running up and down the system that was asked about.
    // Now the lead says it, and says whose turn it is.
    const pending = await this.leadReversalRequestRepository.findOne({
      where: { lead_id: id, status: 'pending' },
      order: { created_at: 'DESC' },
    });

    (lead as any).pending_approval = pending
      ? {
          id: pending.id,
          kind: pending.kind,
          ...
          awaiting_rep_response: pending.awaiting_rep_response,
          enquiry_count: (pending.enquiry_thread ?? []).length,
          // Whose move it is, said plainly so the UI does not have to guess.
          waiting_on: pending.awaiting_rep_response ? 'rep' : 'manager',
        }
      : null;
  ...
  /**
   * The manager's answer to "why were all these leads disqualified".
   *
   * Groups Disqualified leads by their recorded reason so the pattern is
   * visible — 149 "No budget" is a pricing conversation, 22 "Cannot
   * reach contact" is a data-quality one — and reports how many carry no
   * reason at all, which is the backfill queue for the reps. Raw SQL
   * because it is one aggregate over one table; reasons are trimmed so
   * "No budget " and "No budget" count as the same answer.
   */
  async getDisqualificationSummary(): Promise<{
    total: number;
    ...
    const rows: { reason: string; count: number }[] =
      await this.leadRepository.query(`
        SELECT COALESCE(NULLIF(BTRIM(reason), ''), '') AS reason,
               COUNT(*)::int AS count
        FROM leads
        WHERE status = 'Disqualified' AND deleted_at IS NULL
        GROUP BY 1
        ORDER BY count DESC, reason ASC
      `);
    const missing = rows.find((r) => r.reason === '')?.count ?? 0;
    const reasons = rows.filter((r) => r.reason !== '');
    return {
      total: reasons.reduce((s, r) => s + r.count, 0) + missing,
  ...
  async findLeadStakeholders(
    ...
    // Self-heal the lead's own primary contact into a stakeholder.
    //
    // Qualification requires picking a decision maker, and that picker
    // reads this list — so a lead with a contact but no stakeholder row
    // can never be qualified. The rep sees "no stakeholders found, add
    // contacts first" while looking at the contact they already added,
    // and the lead is stuck. Only createLeadWithSchoolAndContacts ever
    // wrote these rows, so every lead from an import, a social handoff
    // or any earlier code path arrived without one — 1,721 of 1,722
    // leads in production.
    //
    // Backfilled as an INFLUENCER, deliberately: promoting a contact to
    // DECISION_MAKER here would silently satisfy a BANT criterion that
    // nobody has actually verified. The rep still nominates the decision
    // maker in the modal; this only makes them selectable.
    const lead = await this.leadRepository.findOne({
      where: { id },
      relations: ['primary_contact'],
    });
    if (!lead?.primary_contact_id) return stakeholders;

    const healed = this.leadStakeholderRepository.create({
      id: uuidv4(),
      lead_id: id,
      contact_id: lead.primary_contact_id,
      role: lead.primary_contact?.role as never,
      decision_role: DecisionRole.INFLUENCER,
      influence_level: InfluenceLevel.MEDIUM,
      is_primary: true,
      notes: '',
    });
    await this.leadStakeholderRepository.save(healed);

    return this.leadStakeholderRepository.find({
      relations: ['contact'],
      where: { lead: { id } },
    });
  }
   ...
   *
   * There was no way to do this at all: the API offered GET and POST and
   * nothing else, so a mistyped name or a changed phone number could only be
   * fixed by removing the person or living with it. Tracker: "Stakeholders
   * details cannot be edited under lead" (20 Aug).
   *
   * Contact-owned fields (name, email, phones, preferred method) are written
   * to the contact; stakeholder-owned fields (role, decision role, influence,
   * primary, notes) to the stakeholder row. Promoting one to primary demotes
   * the others exactly as adding a primary stakeholder does, so the two paths
   * cannot disagree about who the primary contact is.
   */
  async updateStakeholder(
    leadId: string,
    stakeholderId: string,
    dto: UpdateLeadStakeholderDto,
    userId: string,
    scopeUserId?: string,
  ): Promise<LeadStakeholder> {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const stakeholder = await stakeholderRepository.findOne({
        where: { id: stakeholderId, lead_id: lead.id },
        relations: [`contact`],
      });
      if (!stakeholder) {
        throw new NotFoundException(
          `Stakeholder with ID ${stakeholderId} not found on this lead`,
        );
      }

      const contact = await contactRepository.findOne({
        where: { id: stakeholder.contact_id },
      });
      if (!contact) {
        throw new NotFoundException(`Stakeholder contact no longer exists`);
      }
      ...
      if (dto.first_name !== undefined) {
        const value = dto.first_name.trim();
        if (!value) {
          throw new BadRequestException(`first_name cannot be blank`);
        }
        contactUpdates.first_name = value;
      }
      if (dto.last_name !== undefined) {
        const value = dto.last_name.trim();
        if (!value) {
          throw new BadRequestException(`last_name cannot be blank`);
        }
        contactUpdates.last_name = value;
      }
      if (dto.email !== undefined) {
        contactUpdates.email = toNullableString(dto.email);
      }
      if (dto.phone !== undefined) {
        contactUpdates.phone = toNullableString(dto.phone);
      }
      if (dto.secondary_phone !== undefined) {
        contactUpdates.secondary_phone = toNullableString(dto.secondary_phone);
      }
      if (dto.whatsapp_number !== undefined) {
        contactUpdates.whatsapp_number = toNullableString(dto.whatsapp_number);
      }
      if (dto.preferred_contact_method !== undefined) {
        contactUpdates.preferred_contact_method = dto.preferred_contact_method;
      }
      if (dto.role !== undefined) {
        contactUpdates.role = dto.role;
      }
      if (Object.keys(contactUpdates).length > 0) {
        await contactRepository.update(contact.id, contactUpdates);
      }

      // ---- stakeholder-owned fields ---------------------------------
      const stakeholderUpdates: Partial<LeadStakeholder> = {};
      if (dto.role !== undefined) {
        stakeholderUpdates.role = dto.role as any;
      }
      if (dto.decision_role !== undefined) {
        stakeholderUpdates.decision_role = dto.decision_role;
      }
      if (dto.influence_level !== undefined) {
        stakeholderUpdates.influence_level = dto.influence_level;
      }
      if (dto.notes !== undefined) {
        stakeholderUpdates.notes = dto.notes.trim();
      }

      // Promotion to primary demotes every sibling, mirroring addStakeholder.
      // Demotion is deliberately NOT mirrored onto lead.primary_contact_id: a
      // lead with no primary contact breaks qualification, so clearing it has
      // to be done by promoting somebody else.
      if (dto.is_primary === true && !stakeholder.is_primary) {
        ...
        stakeholderUpdates.is_primary = true;
      } else if (dto.is_primary === false && stakeholder.is_primary) {
        stakeholderUpdates.is_primary = false;
      }

      if (Object.keys(stakeholderUpdates).length > 0) {
        await stakeholderRepository.update(stakeholder.id, stakeholderUpdates);
      }

      const updated = await stakeholderRepository.findOne({
        where: { id: stakeholder.id },
        relations: [`contact`],
      });

      const who = [
        updated?.contact?.first_name,
        updated?.contact?.last_name,
      ]
        .filter(Boolean)
        .join(` `);
      ...
      await this.activityLogsService.logUpdate(
        ...
        before,
        {
          ...
          role: updated?.role,
          decision_role: updated?.decision_role,
          influence_level: updated?.influence_level,
          is_primary: updated?.is_primary,
          notes: updated?.notes,
        },
        userId,
        `Updated stakeholder ${who} on lead: ${lead.lead_name}`,
      );

      // Decision-maker changes are Authority evidence, same as on add.
      await this.leadQualificationService.autoTickFromSignals(lead.id, manager);
  ...
  async update(
    ...
    if (
      ...
      // Backfill carve-out: recording WHY on a lead that is already
      // Disqualified is history, not a new kill. The gate exists to stop
      // premature disqualification; blocking the reason after the fact
      // only preserves the mystery managers are trying to clear up.
      // Status cannot move through this branch — it is either absent
      // from the update or already 'Disqualified'.
      const current = await this.leadRepository.findOne({
        where: { id },
        select: ['id', 'status', 'reason'],
      });
      // Narrow on purpose: filling an EMPTY reason only. A rep may not
      // rewrite a reason already on the record — that trail is what a
      // manager reads to judge whether the lead was cut too early, and
      // it must not be editable by the person being judged. Overwriting
      // still goes through a manager or the approval flow.
      const isReasonBackfill =
        current?.status === 'Disqualified' &&
        (current?.reason ?? '').trim() === '' &&
        (requestedStatus === undefined || requestedStatus === 'Disqualified');

      const enforce =
        !isReasonBackfill &&
        (await this.complianceSettings.getBoolean(
  ...
  async createReversalRequest(
    ...
    // Tell the managers there is something waiting. Without this the request
    // sat in a queue nobody was told about, so a rep who disqualified a lead
    // heard nothing back until someone happened to look.
    ...
    await this.notifyAboutRequest({
      userIds: await this.approverIds(),
      title:
        kind === 'tactical_disqualify'
          ? 'Disqualification needs your approval'
          : `${friendlyKind} needs your approval`,
      message:
        kind === 'tactical_disqualify'
          ? `${requesterName} wants to disqualify "${lead.lead_name}". Reason given: ${dto.reason ?? '(none)'}`
          : `${requesterName} raised a ${friendlyKind.toLowerCase()} on "${lead.lead_name}".`,
      leadId: lead.id,
   ...
   */
  /**
   * Everyone who can decide an approval request: admins and sales managers.
   */
  private async approverIds(): Promise<string[]> {
    const rows = await this.dataSource
      ...
      .where('r.name IN (:...roles)', { roles: ['admin', 'sales_manager'] })
  ...
  /**
   * Tell the people who need to act. Never let a notification failure break
   * the thing it is reporting on — the approval itself matters more than the
   * telling, and the request row is the source of truth either way.
   */
  private async notifyAboutRequest(input: {
    userIds: string[];
    title: string;
    message: string;
    leadId: string;
    ...
    if (!recipients.length || !this.userNotifications) return;
    try {
      await this.userNotifications.sendToUsers({
        ...
        dedupeKey: `disqualify-${input.requestId}-${input.dedupeSuffix}`,
        actionUrl: `/leads/${input.leadId}`,
        userIds: recipients,
      });
    } catch (error: any) {
      this.logger.warn(
        `Could not notify about request ${input.requestId}: ${error?.message}`,
      );
    }
  }
  ...
  async raiseEnquiry(
    ...
    const savedEnquiry = await this.leadReversalRequestRepository.save(request);

    // The ball is in the rep's court, so say so.
    await this.notifyAboutRequest({
      userIds: [request.requested_by_id].filter(Boolean) as string[],
      title: 'Your disqualification request needs more information',
      message: `A manager asked: ${message}`,
      leadId: request.lead_id,
      requestId: request.id,
      dedupeSuffix: `enquiry-${savedEnquiry.enquiry_thread.length}`,
    });
  ...
  async respondToEnquiry(
    ...
    const savedResponse = await this.leadReversalRequestRepository.save(request);

    // And back again, so the manager does not have to keep checking.
    await this.notifyAboutRequest({
      userIds: await this.approverIds(),
      title: 'A rep answered your disqualification enquiry',
      message: `Reply: ${message}`,
      leadId: request.lead_id,
      requestId: request.id,
      dedupeSuffix: `reply-${savedResponse.enquiry_thread.length}`,
      severity: 'info',
    });
  ...
  async reviewReversalRequest(
    ...
    if (dto.decision === 'approved') {
      ...
      if (request.kind === 'status_reversal') {
        // DATA-LOSS-REVERSAL: the deletes used to run in their own committed
        // transaction and the status update ran separately afterwards — so if
        // the status update failed (e.g. the requested status re-runs the
        // qualification data check and throws), the deals/invoices/quotes were
        // already permanently gone. Run both in ONE transaction: any failure
        // now rolls the deletes back and leaves the lead untouched.
        await this.dataSource.transaction(async (manager) => {
          await this.removeLeadDealsAndDependents(request.lead_id, manager);
  ...
  // DATA-LOSS-REVERSAL: takes the caller's transaction manager so the deletes
  // commit atomically with the lead status update. Previously this opened its
  // own transaction and committed on its own, which is what created the
  // delete-then-fail data-loss window.
  private async removeLeadDealsAndDependents(
    leadId: string,
  ...
  private async transitionStatus(
    ...
    // FU5 — a lead that has left Nurture has no re-engagement to wait for.
    //
    // Moving a lead to Nurture schedules one "Re-engage: <lead>" task dated
    // months out. Nothing ever closed it again, so a lead that was later
    // qualified, converted or disqualified kept an open task on the books:
    // it counts as outstanding work, shows as the lead's next step, and
    // feeds the follow-up queues. Cancelled rather than deleted, so the
    // decision to nurture stays visible in the record's history.
    //
    // Cancelling also releases the next-step gate: the stale re-engage task
    // was the "future actionable activity" that let a completion through, so
    // leaving it open let discipline pass on a lead nobody is nurturing.
    if (oldStatus === 'Nurture' && status !== 'Nurture') {
      const cancelled = await manager
        .createQueryBuilder()
        .update(Activity)
        .set({ status: ActivityStatus.CANCELLED })
        .where('lead_id = :leadId', { leadId: lead.id })
        .andWhere('type = :type', { type: ActivityType.TASK })
        .andWhere("subject ILIKE 'Re-engage:%'")
        .andWhere('status NOT IN (:...done)', {
          done: [ActivityStatus.COMPLETED, ActivityStatus.CANCELLED],
        })
        .execute();
      ...
      if (cancelled.affected) {
        await this.activityLogsService.logUpdate(
          'Lead',
          lead.id,
          { open_reengagement_tasks: cancelled.affected },
          { open_reengagement_tasks: 0 },
          userId,
          `Cancelled ${cancelled.affected} re-engagement task(s) — lead left Nurture for ${status}`,
        );
      }
    }
  ...
  async assignLead(
    ...
    // The previous owner should stop being nagged about a lead that is no
    // longer theirs. Idle/SLA alerts are personal, so withdraw the ones they
    // have not read yet; anything already read stays as their history. In
    // production this had accumulated: 46,716 unread alerts sat in two
    // people's bells for leads that had since moved to someone else.
    if (oldAssignee && oldAssignee !== assignedTo && this.userNotifications) {
      try {
        const revoked = await this.userNotifications.revokeUnreadForEntity(
          'Lead',
          lead.id,
          [oldAssignee],
          // Owner-only alerts. Escalations and breach alerts are deliberately
          // excluded: they are addressed to managers, not to the owner.
          ['lead-idle-alert-', 'lead-sla-prebreach-', 'demo-followup-sla-'],
        );
        if (revoked) {
          this.logger.log(
            `Withdrew ${revoked} unread alert(s) for lead ${lead.id} from its previous owner`,
          );
        }
      } catch (error: any) {
        // Never let notification hygiene fail an assignment.
        this.logger.warn(
          `Could not withdraw old alerts for lead ${lead.id}: ${error?.message}`,
        );
      }
    }
