...
export class ActivitiesController {
  ...
  async create(
    ...
    // C-02: a rep can only file an activity onto their own lead/deal. His
    // change put userRoles in the argument slot this used to occupy (the
    // completed-create gate needs the roles), so both are passed now.
  ...
  async updateStatus(
    ...
    const activity = await this.activitiesService.updateStatus(
      ...
      userRoles,
      {
        alsoComplete: body.also_complete,
        nextStepExistingId: body.next_step_existing_id,
        scopeUserId,
      },
    );
    return {
