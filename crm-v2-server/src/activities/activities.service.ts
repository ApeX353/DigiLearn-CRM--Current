...
import {
  DealStageAutomationService,
  demoStageTarget,
} from '../deals/deal-stage-automation.service';
...
export class ActivitiesService {
  constructor(
    private readonly dealStageAutomation: DealStageAutomationService,
  ...
  private async assertNextStepCompliance(
    ...
    /** Siblings the SAME request is closing (also_complete / closes) — a
     * step that is about to die cannot stand as the record's future. */
    excludeIds: string[] = [],
  ): Promise<void> {
    ...
    // A terminal record needs no next step — there is nothing left to keep
    // alive. This is what makes the "no next step → disqualify / mark lost"
    // flow legal: the client closes the record FIRST, then completes the
    // activity, and the gate sees the terminal state. (findOne loads the
    // lead/deal relations, so these reads are already paid for.)
    const leadStatus = activity.lead?.status as string | undefined;
    if (leadStatus === 'Disqualified' || leadStatus === 'Converted') return;
    const dealClose = activity.deal?.closeStatus as string | undefined;
    if (dealClose === 'won' || dealClose === 'lost') return;

    // Relationship-terminal outcomes legitimately close a school-level
    // interaction without committing future work — mirror of the client's
    // isRelationshipTerminalOutcome policy.
    const RELATIONSHIP_TERMINAL = new Set<string>([
      'information_shared',
    ...
    if (outcome && RELATIONSHIP_TERMINAL.has(outcome)) return;
    ...
    //
    // It must be DATED. Thousands of legacy rows are logged past events
    // ("Call: no answer") that were stored as status=scheduled with no
    // date at all — records, not plans. Counting those let a lead pass
    // this gate on the strength of a call that already happened while
    // the UI (which only surfaces dated items as the next step) showed
    // "No next step planned". Server and screen now agree.
    const futureCount = await this.activityRepository
      .createQueryBuilder('a')
      .where('a.id NOT IN (:...excluded)', {
        excluded: [activity.id, ...excludeIds],
      })
      .andWhere('a.type IN (:...types)', { types: ACTIONABLE })
      .andWhere("a.status NOT IN ('completed','cancelled')")
      .andWhere('(a.due_at IS NOT NULL OR a.scheduled_at IS NOT NULL)')
      .andWhere(
        new Brackets((qb) => {
          if (activity.lead_id) {
  ...
  /** True when `other` lives on the same lead / deal / contact as `source`. */
  private sharesRecord(
    source: Pick<Activity, 'lead_id' | 'deal_id' | 'contact_id'>,
    other: Pick<Activity, 'lead_id' | 'deal_id' | 'contact_id'>,
  ): boolean {
    if (source.lead_id && other.lead_id === source.lead_id) return true;
    if (source.deal_id && other.deal_id === source.deal_id) return true;
    // Contact-only activities (school pages) share the contact instead —
    // but ONLY with other contact-only rows. A lead's or deal's open work
    // belongs to that record's own next-step gate; letting a school-page
    // call close it through the shared contact would silently remove a
    // lead's only planned step with no gate ever evaluated.
    if (
      !source.lead_id &&
      !source.deal_id &&
      source.contact_id &&
      other.contact_id === source.contact_id &&
      !other.lead_id &&
      !other.deal_id
    ) {
      return true;
  ...
  /**
   * "This also covered…" — mark sibling open activities completed because
   * the interaction being logged/completed settled them too. Runs INSIDE
   * the caller's transaction so the primary completion and the covered
   * ones land together.
   *
   * Rules: every id must be an activity on the SAME record as the source
   * (a rep cannot close another school's work from here); rows already
   * completed/cancelled are skipped as no-ops; the source itself is
   * ignored if included. Each covered row gets the source's outcome and a
   * note that points at the entry holding the real account — the story is
   * told once, where it happened, not copy-pasted onto every row.
   */
  private async closeCoveredActivities(
    ...
    const unique = Array.from(new Set(ids)).filter((id) => id !== source.id);
    if (unique.length === 0) return [];

    const rows = await manager.find(Activity, { where: { id: In(unique) } });
    if (rows.length !== unique.length) {
      const found = new Set(rows.map((r) => r.id));
      const missing = unique.filter((id) => !found.has(id));
      throw new NotFoundException(
        `Activities to close not found: ${missing.join(', ')}`,
      );
    }

    const foreign = rows.filter((r) => !this.sharesRecord(source, r));
    if (foreign.length) {
      throw new BadRequestException(
        'Only open activities on the same lead, deal or contact can be closed as covered by this one.',
      );
    }

    // Only actionable work can be "covered by" a conversation. Notes are
    // records already, and demo rows carry their own lifecycle/derivation
    // that a blanket completion would sidestep.
    const COVERABLE: ActivityType[] = [
      ActivityType.TASK,
      ActivityType.CALL,
      ActivityType.EMAIL,
      ActivityType.MEETING,
    ...
    const notCoverable = rows.filter((r) => !COVERABLE.includes(r.type));
    if (notCoverable.length) {
      throw new BadRequestException(
        'Only tasks, calls, emails, meetings and WhatsApps can be closed as covered.',
      );
    }

    const now = new Date();
    const typeLabel = String(source.type).replace(/_/g, ' ');
    ...
    for (const row of rows) {
      if (
        ...
        continue; // already history — nothing to do
      }
      (row as unknown as { __oldStatus?: ActivityStatus }).__oldStatus =
        row.status;
      row.status = ActivityStatus.COMPLETED;
      row.completed_at = now;
      // The primary's outcome, or nothing. Never a fabricated
      // "successful" — an outcome that wasn't stated isn't known.
      row.completion_outcome = (outcome as ActivityOutcome) ?? null;
      row.completion_note = `Covered by the ${typeLabel} "${source.subject}" logged on ${now.toISOString().slice(0, 10)} — see that entry for what happened.`;
      await manager.save(Activity, row);
      closed.push(row);
    }
    // Audit rows are written by the CALLER after the transaction commits.
    // ActivityLogsService saves through its own repository (a separate
    // connection that commits immediately), so logging in here left
    // "Closed as covered" entries behind whenever a later step rolled the
    // whole transaction back.
    return closed;
  }

  /** Post-commit audit for closeCoveredActivities — see note there. */
  private async auditCoveredClosures(
    closed: Activity[],
    source: Activity,
    userId: string,
  ): Promise<void> {
    ...
    for (const row of closed) {
      await this.activityLogsService.logUpdate(
        ...
        { status: ActivityStatus.COMPLETED, covered_by: source.id },
        userId,
        `Closed as covered by ${typeLabel} "${source.subject}"`,
      );
    }
  }

  /**
   * "The next step is already planned" — the rep points at an existing
   * open, DATED, actionable activity on the same record instead of
   * creating a new one. Validates the nomination; the caller treats a
   * valid nomination exactly like an atomic `next_step` for the gate.
   */
  private async resolveExistingNextStep(
    source: Activity,
    existingId: string,
    excludedIds: string[] = [],
  ): Promise<Activity> {
    if (existingId === source.id) {
      throw new BadRequestException(
        'An activity cannot be its own next step.',
      );
    }
    if (excludedIds.includes(existingId)) {
      throw new BadRequestException(
        'The nominated next step is being closed in this same request — pick one that stays open.',
      );
    }
    ...
    if (!existing) {
      throw new NotFoundException(`Next-step activity ${existingId} not found`);
    }
    if (!this.sharesRecord(source, existing)) {
      throw new BadRequestException(
        'The nominated next step must be on the same lead, deal or contact.',
      );
    }
    const ACTIONABLE: ActivityType[] = [
      ActivityType.TASK,
    ...
    if (!ACTIONABLE.includes(existing.type)) {
      throw new BadRequestException(
        'A note cannot be the next step — nominate a task, call, meeting, email or WhatsApp.',
      );
    }
    if (
      ...
      throw new BadRequestException(
        'The nominated next step is already closed — pick an open one or schedule a new step.',
      );
    }
    if (!existing.due_at && !existing.scheduled_at) {
      throw new BadRequestException(
        'The nominated next step has no date. Give it a date first, or schedule a new step.',
      );
    }
    return existing;
  }
  ...
  /** Where the "next action" date lives differs by type; the sub-row wins. */
  private resolveNextActionDate(dto: CreateActivityDto): string | undefined {
    switch (dto.type) {
      case ActivityType.MEETING:
        return dto.meeting?.start_time ?? dto.due_at;
      case ActivityType.CALL:
        return dto.call?.follow_up_date ?? dto.due_at;
      case ActivityType.WHATSAPP:
        return dto.whatsapp?.follow_up_date ?? dto.due_at;
      case ActivityType.DEMO_BOOKING:
        return dto.demo?.planned_at ?? dto.due_at;
      case ActivityType.DEMO_DELIVERY:
      case ActivityType.DEMO_FOLLOWUP:
        return dto.demo?.next_activity_date ?? dto.due_at;
      default:
        return dto.due_at;
    }
  }

  private assertNextActionDate(dto: CreateActivityDto): void {
    if (dto.type === ActivityType.NOTE) return; // notes are record-keeping only
    // A create that arrives already completed is a RECORD of work that
    // happened ("called the bursar, no answer"), not a plan. Demanding a
    // follow-up date on the record itself forced every logged call to
    // invent a date, and for calls/WhatsApps that date then auto-spawned a
    // "Follow-up:" task — so one phone call produced a follow-up nobody
    // asked for, sitting above the call in the log. The record (lead/deal)
    // still owes a next step; that is decided in the close-the-loop dialog
    // against what the record already has planned, not forced onto the
    // history row.
    if (dto.status === ActivityStatus.COMPLETED) return;
    if (this.resolveNextActionDate(dto)) return;
    throw new BadRequestException(
      'A follow-up date is required for this activity. Set the next action date before saving.',
    );
  }
  ...
  async create(
    ...
    this.assertNextActionDate(dto);

    // closes[] must not remove the record's only planned step. The gate
    // normally runs on the completion PATCH; a status=completed create
    // that closes siblings is a completion in disguise, so it faces the
    // same rule — with the rows it is closing excluded from "the future".
    if (
      dto.status === ActivityStatus.COMPLETED &&
      dto.closes?.length &&
      (dto.lead_id || dto.deal_id)
    ) {
      const pseudo = {
        ...
        lead: dto.lead_id
          ? await this.dataSource.manager.findOne(Lead, {
              where: { id: dto.lead_id },
            })
          : null,
        deal: dto.deal_id
          ? await this.dataSource.manager.findOne(Deal, {
              where: { id: dto.deal_id },
            })
          : null,
      } as unknown as Activity;
      await this.assertNextStepCompliance(
        pseudo,
        false,
        userRoles,
        dto.completion_outcome,
        dto.closes,
      );
    }

    // "Log a past interaction" creates (status=completed) should carry the
    // same outcome discipline as marking an open activity done.
    if (dto.status === ActivityStatus.COMPLETED) {
      const enforce = await this.complianceSettings.getBoolean(
        'enforce_outcome_on_completion',
      );
      if (enforce && (!dto.completion_outcome || !dto.completion_note?.trim())) {
        throw new BadRequestException(
          'completion_outcome and completion_note are required when logging an activity as already completed',
        );
      }
    }

    // A note is a record by definition — there is nothing to "do" about a
    // note later, so it is born completed. Left at the entity default
    // (scheduled) a note sat in the open queue forever: it counted as an
    // open item, could occupy the "Next step" slot, and inflated
    // "+N more open activities" on every record it was written on.
    const effectiveStatus: ActivityStatus | undefined =
      dto.status ??
      (dto.type === ActivityType.NOTE ? ActivityStatus.COMPLETED : undefined);
    const bornCompleted = effectiveStatus === ActivityStatus.COMPLETED;

    const created = await this.dataSource.transaction(async (manager) => {
      // C-02: a rep cannot file an activity onto someone else's record.
      // Kept inside his transaction — it is orthogonal to the change he
      // is making and costs his behaviour nothing.
      ...
      // A plan's date can arrive only in its type-specific field (the Log
      // Activity modal sends a meeting's date as meeting.start_time and
      // nothing else). Mirror it onto the row itself, because everything
      // that decides "is there a next step" — the compliance gate, the
      // Already-planned picker, the Planned slot — reads due_at /
      // scheduled_at, not the sub-row.
      const mirroredNextAt =
        !bornCompleted && !dto.due_at && !dto.scheduled_at
          ? this.resolveNextActionDate(dto)
          : undefined;
      ...
      const activity = manager.create(Activity, {
        ...
        completed_at: bornCompleted ? new Date() : undefined,
        // Outcome fields only make sense on a completed create; strip them
        // from scheduled creates so a stray payload can't pre-fill an
        // outcome on work that hasn't happened yet.
        // NOTE: this supersedes ACT4, which kept a logged interaction's own
        // past date as the completion moment. His model is that a logged
        // record is stamped when it is logged — the same rule migration
        // 1772 applies to the legacy rows (completed_at = created_at).
        completion_outcome:
          dto.status === ActivityStatus.COMPLETED
            ? dto.completion_outcome
            : undefined,
        completion_note:
          dto.status === ActivityStatus.COMPLETED
            ? dto.completion_note?.trim() || undefined
            : undefined,
      });

      const savedActivity = await manager.save(Activity, activity);

      // One conversation, one line of communication. A logged call can
      // close the open items it covered — the scheduled meeting that
      // actually took place, the "follow up on training" task the same
      // call settled — so reps stop leaving held meetings to go overdue
      // and stop running parallel follow-up chains for one relationship.
      if (bornCompleted && closes?.length) {
        const closedRows = await this.closeCoveredActivities(
          manager,
          closes,
          savedActivity,
          userId,
        ...
        (savedActivity as unknown as { __closedCovered?: Activity[] }).__closedCovered =
          closedRows;
      }
      ...
      switch (dto.type) {
        ...
        case ActivityType.EMAIL:
          if (email) {
            ...
            // A logged email is a RECORD of one already sent by hand —
            // dispatching it again would email the school twice (and
            // "log what happened" must never cause outbound mail). Only
            // planned/sending creates go through the sender.
            if (!bornCompleted) {
    ...
    // Covered-closure audit — post-commit, so a rolled-back transaction
    // never leaves "Closed as covered" entries for rows that reopened.
    const closedCovered = (created as unknown as { __closedCovered?: Activity[] })
      .__closedCovered;
    if (closedCovered?.length) {
      await this.auditCoveredClosures(closedCovered, created, userId);
    }

    // Stage automation (recording rule 3): recording a demo on a deal IS
    // the stage event, so the board moves without a drag. Runs AFTER the
    // transaction commits — the automation must never be able to roll
    // back the activity the rep just logged.
    //
    // Restored 25 Aug once 07ab979 was ported; it was stripped when his
    // 23/24 Aug work landed first and these symbols did not yet exist here.
    if (created.deal_id) {
      const target = demoStageTarget(
        created.type,
        created.status,
        created.completion_outcome,
      );
      if (target) {
        await this.dealStageAutomation.advance(
          created.deal_id,
          target,
          userId,
          `demo ${target === 'demo_completed' ? 'delivered' : 'booked'} (${created.subject})`,
        );
      }
    }
  ...
  async updateStatus(
    ...
    userRoles: string[] = [],
    extras: {
      /** Sibling open activities this same interaction covered — closed in the same transaction. */
      alsoComplete?: string[];
      /** An existing open dated activity that IS the next step — satisfies the gate without creating a row. */
      nextStepExistingId?: string;
      /** C-02: set for a sales_rep so they can only transition their own work. */
      scopeUserId?: string;
    } = {},
  ): Promise<Activity> {
    // C-02: a rep can only transition an activity in their own scope.
    const activity = await this.findOne(id, extras.scopeUserId);
    const oldStatus = activity.status;
    const alsoComplete = (extras.alsoComplete ?? []).filter((x) => x !== id);

    // "Already planned": validate the nomination up front so a bad id
    // never leaves the activity half-completed. A valid nomination counts
    // exactly like an atomic next_step for the compliance gate below.
    const existingNextStep =
      status === ActivityStatus.COMPLETED && extras.nextStepExistingId
        ? await this.resolveExistingNextStep(
            activity,
            extras.nextStepExistingId,
            alsoComplete,
          )
        : null;
    ...
    // The outcome dropdown alone is a rubber stamp — the note is the part
    // a manager can actually read. Same enforcement tier as the outcome:
    // unconditional on this path, because this is the path the UI uses.
    if (status === ActivityStatus.COMPLETED && !completionNote?.trim()) {
      throw new BadRequestException(
        'completion_note is required when marking an activity completed — record what happened',
      );
    }
    ...
    } else if (oldStatus === ActivityStatus.COMPLETED) {
      // Reopening. A stale completed_at makes the row invisible to the
      // open_only filter AND makes the client's completion safety net
      // think the activity just completed — which re-opened the
      // un-dismissable next-step dialog on every reopen. The outcome
      // fields go with it: a reopened activity has no outcome yet, and
      // the old one survives in the audit log.
      activity.completed_at = null as unknown as Date;
      activity.completion_outcome = null as unknown as ActivityOutcome;
      activity.completion_note = null;
    }

    // Completion and its next_step commit in ONE transaction. The old
    // shape saved the parent, then inserted the follow-up in a swallowed
    // try/catch — so a failed insert produced a completed activity with
    // no scheduled future while the client (which suppresses its safety
    // net whenever it sent next_step) toasted success. Now either both
    // land or neither does, which is the contract the close-the-loop
    // dialog is built on. Contact-only activities count as parented:
    // their follow-up carries the same contact_id.
    const scheduleNextStep =
      status === ActivityStatus.COMPLETED &&
      !!nextStep &&
      !!(activity.lead_id || activity.deal_id || activity.contact_id);

    let coveredClosed: Activity[] = [];
    await this.dataSource.transaction(async (manager) => {
      await manager.save(Activity, activity);
      // Siblings this conversation also covered close in the same
      // transaction — one outcome, one account, one line of communication.
      if (status === ActivityStatus.COMPLETED && alsoComplete.length) {
        coveredClosed = await this.closeCoveredActivities(
          manager,
          alsoComplete,
          activity,
          userId,
      ...
      if (scheduleNextStep && nextStep) {
        const followUp = manager.create(Activity, {
          type: nextStep.type,
        ...
        await manager.save(Activity, followUp);
      }
    });

    // Covered-closure audit — post-commit, so a rollback never leaves
    // "Closed as covered" entries for rows that reopened.
    if (coveredClosed.length) {
      await this.auditCoveredClosures(coveredClosed, activity, userId);
    }
    ...
    await this.activityLogsService.logUpdate(
      ...
      { status: oldStatus },
      {
        ...
        ...(status === ActivityStatus.COMPLETED
          ...
          : {}),
        ...(existingNextStep ? { next_step_existing_id: existingNextStep.id } : {}),
        ...(alsoComplete.length ? { also_completed: alsoComplete } : {}),
      },
      userId,
      existingNextStep
        ? `Updated activity status from ${oldStatus} to ${status} — next step already planned: "${existingNextStep.subject}"`
    ...
    // Stage automation (recording rule 3). Completing a demo *delivery*
    // proves Demo Completed; completing a demo *booking* only proves the
    // demo is booked — marking the booking call done a week early must
    // not jump the board to Demo Completed. An outcome that says the
    // demo never ran (rescheduled, no response) advances nothing.
    //
    // Restored 25 Aug with 07ab979.
    if (
      activity.deal_id &&
      status === ActivityStatus.COMPLETED &&
      oldStatus !== ActivityStatus.COMPLETED
    ) {
      const target = demoStageTarget(activity.type, status, outcome);
      if (target) {
        await this.dealStageAutomation.advance(
          activity.deal_id,
          target,
          userId,
          `demo ${target === 'demo_completed' ? 'completed' : 'booked'} (${activity.subject})`,
        );
      }
    }
