...
export class DocumentGeneratorService {
  ...
  /**
   * The company block on documents now comes from Settings → General
   * (company_name / email / phone / website), falling back to the env-based
   * defaults. This is what makes the "Company Information" settings actually
   * appear on invoices and quotes instead of being stored-but-unused.
   */
  private async resolveCompany(): Promise<CompanyInfo> {
    const read = async (key: string): Promise<string> => {
      const s = await this.appSettingsService.getSetting(key);
      return typeof s?.value === 'string' ? s.value.trim() : '';
    };
    const [name, email, phone, website] = await Promise.all([
      read('company_name'),
      read('company_email'),
      read('company_phone'),
      read('company_website'),
    ]);
    return {
      ...
      website: website || this.company.website,
    };
  }
  ...
  async generateInvoicePdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Header
      this.drawHeader(doc, 'INVOICE', company);
  ...
  async generateQuotePdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Header
      this.drawHeader(doc, 'QUOTATION', company);
  ...
  private drawHeader(
    ...
    let companyY = 75;
    if (company.address) {
      doc.text(company.address, 50, companyY);
      companyY += 12;
    }
    if (company.email) {
      doc.text(company.email, 50, companyY);
      companyY += 12;
    }
    if (company.phone) {
      doc.text(company.phone, 50, companyY);
      companyY += 12;
    }
    if (company.website) {
      doc.text(company.website, 50, companyY);
    }
